...
def customShadowCallback_Shadow(payload, responseStatus, token):
    # payload is a JSON string ready to be parsed using json.loads(...)
    # in both Py2.x and Py3.x
    print(responseStatus)
    payloadDict = json.loads(payload)
    ...
    ws.send(str(payloadDict["state"]["reported"]))
    # response = ws.recv()
    ws.close()
    print("+++++++++++++++++++++++\n\n")
...
while True:
    deviceShadowHandler.shadowGet(customShadowCallback_Shadow, 5)
    time.sleep(5)
